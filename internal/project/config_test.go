...
// configWithActiveSections returns a config string that has active [agent] and
// [multiplexer] sections so tests can exercise the common update path.
func configWithActiveSections(agentType, multiplexerType string) string {
...
func configWithCommentedAgentSection(multiplexerType string) string {
	return `# Sarge Project Configuration
...
func TestUpdateConfigFields_UpdateActiveAgentType(t *testing.T) {
	...
	err := UpdateConfigFields(configPath, "claude", "zellij")
	require.NoError(t, err)
...
func TestUpdateConfigFields_UpdateActiveMultiplexerType(t *testing.T) {
	cfg := configWithActiveSections("pi", "zellij")
	...
	err := UpdateConfigFields(configPath, "pi", "zmx")
	require.NoError(t, err)
	...
	require.Contains(t, content, `type = "zmx"`, "multiplexer type should be updated to zmx")
	require.NotContains(t, content, `type = "zellij"`, "old multiplexer type should be replaced")
}

func TestUpdateConfigFields_BothSectionsMissing_NoFailure(t *testing.T) {
	cfg := `[project]
name = "test-project"
`
	tmpDir := t.TempDir()
...
func TestUpdateConfigFields_CommentedAgentSection_ActivatedWhenSwitchingToPI(t *testing.T) {
	// This is the critical bug scenario (main-u4b0): the default claude config
	// has [agent] commented out. Switching to "pi" must uncomment it and write the type.
	cfg := configWithCommentedAgentSection("zellij")

	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "config.toml")
	require.NoError(t, os.WriteFile(configPath, []byte(cfg), 0600))

	err := UpdateConfigFields(configPath, "pi", "zellij")
	require.NoError(t, err)
...
`
	...
	require.Contains(t, content, `another = "also untouched"`)
	// Multiplexer type updated
